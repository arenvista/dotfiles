

# Tue Jan 27 07:12:31 PM EST 2026 - Unknown packages


AddPackage alsa-utils # Advanced Linux Sound Architecture - Utilities
AddPackage base # Minimal package set to define a basic Arch Linux installation
AddPackage base-devel # Basic tools to build Arch Linux packages
AddPackage bitwarden-cli # The command line vault
AddPackage blueman # GTK+ Bluetooth Manager
AddPackage bluez-utils # Development and debugging utilities for the bluetooth protocol stack
AddPackage bpytop # Resource monitor that shows usage and stats for processor, memory, disks, network and processes
AddPackage brightnessctl # Lightweight brightness control tool
AddPackage calc # Arbitrary precision console calculator
AddPackage cava # Console-based Audio Visualizer with support for multiple backends
AddPackage discord # All-in-one voice and text chat for gamers
AddPackage dmidecode # Desktop Management Interface table related utilities
AddPackage dolphin # KDE File Manager
AddPackage dunst # Customizable and lightweight notification-daemon
AddPackage easyeffects # Audio Effects for Pipewire applications
AddPackage efibootmgr # Linux user-space application to modify the EFI Boot Manager
AddPackage fd # Simple, fast and user-friendly alternative to find
AddPackage feh # Fast and light imlib2-based image viewer
AddPackage firefox # Fast, Private & Safe Web Browser
AddPackage firefox-tridactyl # Replace Firefox's control mechanism with one modelled on Vim
AddPackage flatpak # Linux application sandboxing and distribution framework (formerly xdg-app)
AddPackage fwupd # Simple daemon to allow session software to update firmware
AddPackage fzf # Command-line fuzzy finder
AddPackage gcr # A library for bits of crypto UI and parsing
AddPackage gdb # The GNU Debugger
AddPackage geckodriver # Proxy for using W3C WebDriver-compatible clients to interact with Gecko-based browsers.
AddPackage ghc # The Glasgow Haskell Compiler
AddPackage gimp # GNU Image Manipulation Program
AddPackage git # the fast distributed version control system
AddPackage glew # The OpenGL Extension Wrangler Library
AddPackage glm # C++ mathematics library for 3D software based on the OpenGL Shading Language (GLSL) specification
AddPackage grim # Screenshot utility for Wayland
AddPackage gst-plugin-pipewire # Multimedia graph framework - pipewire plugin
AddPackage htop # Interactive process viewer
AddPackage hunspell-en_us # US English hunspell dictionaries
AddPackage hyprland # a highly customizable dynamic tiling Wayland compositor
AddPackage imagemagick # An image viewing/manipulation program
AddPackage intel-media-driver # Intel Media Driver for VAAPI — Broadwell+ iGPUs
AddPackage intel-ucode # Microcode update files for Intel CPUs
AddPackage iwd # Internet Wireless Daemon
AddPackage jq # Command-line JSON processor
AddPackage kaddressbook # KDE contact manager
AddPackage kdeconnect # Adds communication between KDE and your smartphone
AddPackage kitty # A modern, hackable, featureful, OpenGL-based terminal emulator
AddPackage libpulse # A featureful, general-purpose sound server (client library)
AddPackage libreoffice-fresh # LibreOffice branch which contains new features and program enhancements
AddPackage libva-intel-driver # VA-API implementation for Intel G45 and HD Graphics family
AddPackage linux # The Linux kernel and modules
AddPackage linux-firmware # Firmware files for Linux - Default set
AddPackage lsd # Modern ls with a lot of pretty colors and awesome icons
AddPackage lsp-plugins # Collection of open-source plugins
AddPackage luarocks # Deployment and management system for Lua modules
AddPackage man-db # A utility for reading man pages
AddPackage mpv # a free, open source, and cross-platform media player
AddPackage nano # Pico editor clone with enhancements
AddPackage nasm # 80x86 assembler designed for portability and modularity
AddPackage ncdu # Disk usage analyzer with an ncurses interface
AddPackage ncspot # Cross-platform ncurses Spotify client written in Rust, inspired by ncmpc and the likes.
AddPackage neovim # Fork of Vim aiming to improve user experience, plugins, and GUIs
AddPackage network-manager-applet # Applet for managing network connections
AddPackage networkmanager # Network connection manager and user applications
AddPackage nnn # The fastest terminal file manager ever written.
AddPackage npm # JavaScript package manager
AddPackage obs-studio # Free, open source software for live streaming and recording
AddPackage obsidian # A powerful knowledge base that works on top of a local folder of plain text Markdown files
AddPackage pacman-contrib # Contributed scripts and tools for pacman systems
AddPackage pandoc-cli # Conversion between documentation formats
AddPackage patchelf # Small utility to modify the dynamic linker and RPATH of ELF executables
AddPackage pavucontrol # PulseAudio Volume Control
AddPackage pdfgrep # A tool to search text in PDF files
AddPackage pipewire # Low-latency audio/video router and processor
AddPackage pipewire-alsa # Low-latency audio/video router and processor - ALSA configuration
AddPackage pipewire-jack # Low-latency audio/video router and processor - JACK replacement
AddPackage pipewire-pulse # Low-latency audio/video router and processor - PulseAudio replacement
AddPackage powertop # A tool to diagnose issues with power consumption and power management
AddPackage qt5-wayland # Provides APIs for Wayland
AddPackage qt5ct # Qt5 Configuration Utility
AddPackage qt6-wayland # Provides APIs for Wayland
AddPackage qt6ct # Qt 6 Configuration Utility
AddPackage raylib # Simple and easy-to-use game programming library
AddPackage ripgrep # A search tool that combines the usability of ag with the raw speed of grep
AddPackage rsync # A fast and versatile file copying tool for remote and local files
AddPackage rustup # The Rust toolchain installer
AddPackage s-tui # Terminal UI stress test and monitoring tool
AddPackage scrcpy # Display and control your Android device
AddPackage sddm # QML based X11 and Wayland display manager
AddPackage sdl12-compat # SDL 1.2 runtime compatibility library using SDL 2.0
AddPackage sfml # A simple, fast, cross-platform, and object-oriented multimedia API
AddPackage slurp # Select a region in a Wayland compositor
AddPackage smartmontools # Control and monitor S.M.A.R.T. enabled ATA and SCSI Hard Drives
AddPackage spotify-launcher # Client for spotify's apt repository in Rust for Arch Linux
AddPackage spotify-player # A command driven spotify player
AddPackage steam # Valve's digital software delivery system
AddPackage stow # Manage installation of multiple softwares in the same directory tree
AddPackage stress # A tool that stress tests your system (CPU, memory, I/O, disks)
AddPackage swaylock # Screen locker for Wayland
AddPackage swww # A Solution to your Wayland Wallpaper Woes
AddPackage syncthing # Open Source Continuous Replication / Cluster Synchronization Thing
AddPackage texlive-bibtexextra # TeX Live - BibTeX additional styles
AddPackage texlive-binextra # TeX Live - TeX auxiliary programs
AddPackage texlive-context # TeX Live - ConTeXt and packages
AddPackage texlive-fontsextra # TeX Live - Additional fonts
AddPackage texlive-fontsrecommended # TeX Live - Recommended fonts
AddPackage texlive-fontutils # TeX Live - Graphics and font utilities
AddPackage texlive-formatsextra # TeX Live - Additional formats
AddPackage texlive-games # TeX Live - Games typesetting
AddPackage texlive-humanities # TeX Live - Humanities packages
AddPackage texlive-latex # TeX Live - LaTeX fundamental packages
AddPackage texlive-latexextra # TeX Live - LaTeX additional packages
AddPackage texlive-latexrecommended # TeX Live - LaTeX recommended packages
AddPackage texlive-luatex # TeX Live - LuaTeX packages
AddPackage texlive-mathscience # TeX Live - Mathematics, natural sciences, computer science packages
AddPackage texlive-metapost # TeX Live - MetaPost and Metafont packages
AddPackage texlive-music # TeX Live - Music packages
AddPackage texlive-pictures # TeX Live - Graphics, pictures, diagrams
AddPackage texlive-plaingeneric # TeX Live - Plain (La)TeX packages
AddPackage texlive-pstricks # TeX Live - PSTricks
AddPackage texlive-publishers # TeX Live - Publisher styles, theses, etc.
AddPackage texlive-xetex # TeX Live - XeTeX and packages
AddPackage throttled # Workaround for Intel throttling issues in Linux.
AddPackage timeshift # A system restore utility for Linux
AddPackage tlp # Linux Advanced Power Management
AddPackage tlp-rdw # Linux Advanced Power Management - Radio Device Wizard
AddPackage tmux # Terminal multiplexer
AddPackage trash-cli # Command line trashcan (recycle bin) interface
AddPackage tree # A directory listing program displaying a depth indented list of files
AddPackage tree-sitter-cli # CLI tool for developing, testing, and using Tree-sitter parsers
AddPackage ttf-jetbrains-mono-nerd # Patched font JetBrains Mono from nerd fonts library
AddPackage ttf-nerd-fonts-symbols # High number of extra glyphs from popular 'iconic fonts'
AddPackage unzip # For extracting and viewing files in .zip archives
AddPackage valgrind # Tool to help find memory-management problems in programs
AddPackage vim # Vi Improved, a highly configurable, improved version of the vi text editor
AddPackage viu # Simple terminal image viewer
AddPackage vulkan-extra-layers # Extra layers for Vulkan development
AddPackage vulkan-extra-tools # Vulkan Configurator and Installation Analyzer
AddPackage vulkan-html-docs # Vulkan and Vulkan SC API Documentation - HTML
AddPackage vulkan-intel # Open-source Vulkan driver for Intel GPUs
AddPackage vulkan-radeon # Open-source Vulkan driver for AMD GPUs
AddPackage vulkan-tools # Vulkan tools and utilities
AddPackage vulkan-utility-libraries # Vulkan Utility Libraries
AddPackage waybar # Highly customizable Wayland bar for Sway and Wlroots based compositors
AddPackage wayvnc # VNC server for wlroots-based Wayland compositors
AddPackage wget # Network utility to retrieve files from the web
AddPackage wine # A compatibility layer for running Windows programs
AddPackage wireless_tools # Tools allowing to manipulate the Wireless Extensions
AddPackage wireplumber # Session / policy manager implementation for PipeWire
AddPackage wireshark-qt # Network traffic and protocol analyzer/sniffer - Qt GUI
AddPackage wl-clipboard # Command-line copy/paste utilities for Wayland
AddPackage wofi # launcher for wlroots-based wayland compositors
AddPackage x11vnc # VNC server for real X displays
AddPackage xdg-desktop-portal-gtk # A backend implementation for xdg-desktop-portal using GTK
AddPackage xdg-desktop-portal-hyprland # xdg-desktop-portal backend for hyprland
AddPackage xdg-utils # Command line tools that assist applications with a variety of desktop integration tasks
AddPackage xf86-video-amdgpu # X.org amdgpu video driver
AddPackage xf86-video-ati # X.org ati video driver
AddPackage xf86-video-dummy # X.org dummy video driver
AddPackage xf86-video-nouveau # Open Source 3D acceleration driver for nVidia cards
AddPackage xorg-server # Xorg X server
AddPackage xorg-xev # Print contents of X events
AddPackage xorg-xinit # X.Org initialisation program
AddPackage yazi # Blazing fast terminal file manager written in Rust, based on async I/O
AddPackage yt-dlp # A youtube-dl fork with additional features and fixes
AddPackage zathura # Minimalistic document viewer
AddPackage zathura-pdf-poppler # Adds pdf support to zathura by using the poppler engine
AddPackage zram-generator # Systemd unit generator for zram devices
AddPackage zsh # A very advanced and programmable command interpreter (shell) for UNIX


# Tue Jan 27 07:12:44 PM EST 2026 - Unknown foreign packages


AddPackage --foreign aconfmgr-git # A configuration manager for Arch Linux
AddPackage --foreign acpilight # A backward-compatible xbacklight replacement based on ACPI
AddPackage --foreign anki # Helps you remember facts (like words/phrases in a foreign language) efficiently
AddPackage --foreign bibata-cursor-theme-bin # Material Based Cursor Theme
AddPackage --foreign catnap-git # A minimalistic and fast system fetch written in Nim.
AddPackage --foreign cbonsai # A bonsai tree generator, written in C using ncurses
AddPackage --foreign deskreen # Turn any device into a secondary screen for your computer
AddPackage --foreign downgrade # Bash script for downgrading one or more packages to a version in your cache or the A.L.A.
AddPackage --foreign hyprland-workspaces # A multi-monitor aware Hyprland workspace widget for use with bars.
AddPackage --foreign kpeoplevcard-git # Expose VCard contacts to KPeople
AddPackage --foreign rofi-lbonn-wayland # A window switcher, run dialog and dmenu replacement - fork with wayland support
AddPackage --foreign rose-pine-hyprcursor # None
AddPackage --foreign sddm-sugar-dark # The sweetest dark theme around for SDDM, the Simple Desktop Display Manager.
AddPackage --foreign ttf-nonicons-bin-git # A next-generation icon set for developers that extends octicons.
AddPackage --foreign visual-studio-code-bin # Visual Studio Code (vscode): Editor for building and debugging modern web and cloud applications (official binary version)
AddPackage --foreign webcord # A Discord client implemented directly without Discord API
AddPackage --foreign wl-kbptr # Control the mouse pointer with the keyboard on Wayland
AddPackage --foreign wlogout # Logout menu for wayland
AddPackage --foreign wlrctl # Utility for miscellaneous wlroots extensions
AddPackage --foreign wluma # Automatic brightness adjustment based on screen contents and ALS
AddPackage --foreign xf86-video-vmware # X.org vmware video driver
AddPackage --foreign yay # Yet another yogurt. Pacman wrapper and AUR helper written in go.
AddPackage --foreign yay-debug # Detached debugging symbols for yay


# Tue Jan 27 07:12:47 PM EST 2026 - New / changed files


CopyFile /boot/EFI/BOOT/BOOTX64.EFI 755
CreateDir /boot/EFI/Linux
CreateDir /boot/EFI/arch/fw
CopyFile /boot/EFI/arch/fwupdx64.efi 755
CopyFile /boot/EFI/systemd/systemd-bootx64.efi 755
CopyFile /boot/initramfs-linux.img 755
CopyFile /boot/loader/entries.srel 755
CopyFile /boot/loader/entries/2024-04-19_22-14-13_linux-fallback.conf 755
CopyFile /boot/loader/entries/2024-04-19_22-14-13_linux.conf 755
CopyFile /boot/loader/loader.conf 755
CopyFile /boot/loader/random-seed 755
CreateFile /etc/.pwd.lock 600 > /dev/null
CopyFile /etc/.updated
CopyFile /etc/NetworkManager/system-connections/1808.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/42325RiverWinds-5.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/A201NE_1f1350.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/AA-Inflight.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/ATTE2W5ruI.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/AoK.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/Aren\'s\ Z\ Fold5.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/Conquest\ Guest.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/EJayla.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/Fios-D4T6L-5G.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/Fios-DM4vn.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/GOBILLS-5G.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/GlobeAtHome_0BC30_5.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/HANEDA-FREE-WIFI\(2.4G\).nmconnection 600
CopyFile /etc/NetworkManager/system-connections/Japan\ Airlines.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/Note20.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/Snorlax_Belly_Drum5g.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/SpectrumSetup-A2.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/SpiritWiFi.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/TMOBILE-3C91.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/TravelerConGuestWiFi.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/Twoseasons.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/UMBC\ Visitor.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/Unitedwifi.com.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/Verizon_NJRT9P.nmconnection 600
CopyFile /etc/NetworkManager/system-connections/clairhomefiber.nmconnection 600
CopyFile /etc/X11/xorg.conf.d/00-keyboard.conf
CreateDir /etc/audisp
CreateDir /etc/audit/plugins.d 750
CreateDir /etc/audit/rules.d
CopyFile /etc/bluetooth/main.conf
CreateDir /etc/cni/net.d 700
CreateLink /etc/fonts/conf.d/10-hinting-slight.conf /usr/share/fontconfig/conf.default/10-hinting-slight.conf
CreateLink /etc/fonts/conf.d/10-scale-bitmap-fonts.conf /usr/share/fontconfig/conf.default/10-scale-bitmap-fonts.conf
CreateLink /etc/fonts/conf.d/10-yes-antialias.conf /usr/share/fontconfig/conf.default/10-yes-antialias.conf
CreateLink /etc/fonts/conf.d/11-lcdfilter-default.conf /usr/share/fontconfig/conf.default/11-lcdfilter-default.conf
CreateLink /etc/fonts/conf.d/20-unhint-small-vera.conf /usr/share/fontconfig/conf.default/20-unhint-small-vera.conf
CreateLink /etc/fonts/conf.d/30-metric-aliases.conf /usr/share/fontconfig/conf.default/30-metric-aliases.conf
CreateLink /etc/fonts/conf.d/30-win32-aliases.conf /usr/share/fontconfig/conf.default/30-win32-aliases.conf
CreateLink /etc/fonts/conf.d/40-nonlatin.conf /usr/share/fontconfig/conf.default/40-nonlatin.conf
CreateLink /etc/fonts/conf.d/45-generic.conf /usr/share/fontconfig/conf.default/45-generic.conf
CreateLink /etc/fonts/conf.d/45-latin.conf /usr/share/fontconfig/conf.default/45-latin.conf
CreateLink /etc/fonts/conf.d/48-spacing.conf /usr/share/fontconfig/conf.default/48-spacing.conf
CreateLink /etc/fonts/conf.d/49-sansserif.conf /usr/share/fontconfig/conf.default/49-sansserif.conf
CreateLink /etc/fonts/conf.d/50-user.conf /usr/share/fontconfig/conf.default/50-user.conf
CreateLink /etc/fonts/conf.d/51-local.conf /usr/share/fontconfig/conf.default/51-local.conf
CreateLink /etc/fonts/conf.d/60-generic.conf /usr/share/fontconfig/conf.default/60-generic.conf
CreateLink /etc/fonts/conf.d/60-latin.conf /usr/share/fontconfig/conf.default/60-latin.conf
CreateLink /etc/fonts/conf.d/65-fonts-persian.conf /usr/share/fontconfig/conf.default/65-fonts-persian.conf
CreateLink /etc/fonts/conf.d/65-nonlatin.conf /usr/share/fontconfig/conf.default/65-nonlatin.conf
CreateLink /etc/fonts/conf.d/69-unifont.conf /usr/share/fontconfig/conf.default/69-unifont.conf
CreateLink /etc/fonts/conf.d/80-delicious.conf /usr/share/fontconfig/conf.default/80-delicious.conf
CreateLink /etc/fonts/conf.d/90-synthetic.conf /usr/share/fontconfig/conf.default/90-synthetic.conf
CopyFile /etc/fstab
CopyFile /etc/fwupd/remotes.d/lvfs-testing.conf
CopyFile /etc/fwupd/remotes.d/lvfs.conf
CopyFile /etc/group
CopyFile /etc/group-
CopyFile /etc/gshadow
CopyFile /etc/gshadow- 600
CopyFile /etc/hostname
CreateDir /etc/iwd
CopyFile /etc/ld.so.cache
CopyFile /etc/locale.conf
CopyFile /etc/locale.gen
CopyFile /etc/locale.gen.pacnew
CreateLink /etc/localtime /usr/share/zoneinfo/America/New_York
CopyFile /etc/machine-id 444
CopyFile /etc/makepkg.conf.d/fortran.conf
CopyFile /etc/makepkg.conf.d/fortran.conf.pacnew
CopyFile /etc/makepkg.conf.d/rust.conf
CopyFile /etc/makepkg.conf.d/rust.conf.pacnew
CopyFile /etc/mkinitcpio.conf
CopyFile /etc/mkinitcpio.conf.pacnew
CopyFile /etc/mkinitcpio.d/linux.preset
CopyFile /etc/modprobe.d/psmouse.conf
CreateLink /etc/os-release ../usr/lib/os-release
CopyFile /etc/pacman.conf
CopyFile /etc/pacman.conf.pacnew
CreateFile /etc/pacman.d/gnupg/.gpg-v21-migrated > /dev/null
CopyFile /etc/pacman.d/gnupg/crls.d/DIR.txt
CopyFile /etc/pacman.d/gnupg/gpg-agent.conf
CopyFile /etc/pacman.d/gnupg/gpg.conf
CopyFile /etc/pacman.d/gnupg/openpgp-revocs.d/45078CCB327E847A9ECFCA1CC01FAD9E22C03232.rev 600
CopyFile /etc/pacman.d/gnupg/private-keys-v1.d/FE5E4D6C4AA1AFDE493F9BEF0BFF896829F22FB9.key 600
CopyFile /etc/pacman.d/gnupg/pubring.gpg
CopyFile /etc/pacman.d/gnupg/pubring.gpg~
CreateFile /etc/pacman.d/gnupg/secring.gpg 600 > /dev/null
CopyFile /etc/pacman.d/gnupg/tofu.db
CopyFile /etc/pacman.d/gnupg/trustdb.gpg
CopyFile /etc/pacman.d/mirrorlist
CopyFile /etc/pacman.d/mirrorlist.backup
CopyFile /etc/pacman.d/mirrorlist.pacnew
CopyFile /etc/pacman.d/ml
CopyFile /etc/passwd
CopyFile /etc/passwd-
CopyFile /etc/resolv.conf
CopyFile /etc/sddm.conf.d/default.conf
CopyFile /etc/shadow
CopyFile /etc/shadow- 600
CopyFile /etc/shells
CopyFile /etc/ssh/ssh_host_ecdsa_key 600
CopyFile /etc/ssh/ssh_host_ecdsa_key.pub
CopyFile /etc/ssh/ssh_host_ed25519_key 600
CopyFile /etc/ssh/ssh_host_ed25519_key.pub
CopyFile /etc/ssh/ssh_host_rsa_key 600
CopyFile /etc/ssh/ssh_host_rsa_key.pub
CopyFile /etc/subgid
CreateFile /etc/subgid- > /dev/null
CopyFile /etc/subuid
CreateFile /etc/subuid- > /dev/null
CopyFile /etc/sudoers.d/00_sybil 440
CopyFile /etc/systemd/system/bluetooth.service
CreateLink /etc/systemd/system/bluetooth.target.wants/bluetooth.service /usr/lib/systemd/system/bluetooth.service
CreateLink /etc/systemd/system/dbus-org.bluez.service /usr/lib/systemd/system/bluetooth.service
CreateLink /etc/systemd/system/dbus-org.freedesktop.nm-dispatcher.service /usr/lib/systemd/system/NetworkManager-dispatcher.service
CreateLink /etc/systemd/system/dbus-org.freedesktop.timesync1.service /usr/lib/systemd/system/systemd-timesyncd.service
CreateLink /etc/systemd/system/display-manager.service /usr/lib/systemd/system/sddm.service
CreateLink /etc/systemd/system/getty.target.wants/getty@tty1.service /usr/lib/systemd/system/getty@.service
CreateLink /etc/systemd/system/multi-user.target.wants/NetworkManager.service /usr/lib/systemd/system/NetworkManager.service
CreateLink /etc/systemd/system/multi-user.target.wants/remote-fs.target /usr/lib/systemd/system/remote-fs.target
CreateLink /etc/systemd/system/multi-user.target.wants/throttled.service /usr/lib/systemd/system/throttled.service
CreateLink /etc/systemd/system/multi-user.target.wants/tlp.service /usr/lib/systemd/system/tlp.service
CreateLink /etc/systemd/system/network-online.target.wants/NetworkManager-wait-online.service /usr/lib/systemd/system/NetworkManager-wait-online.service
CopyFile /etc/systemd/system/powertop.service
CreateLink /etc/systemd/system/sockets.target.wants/systemd-userdbd.socket /usr/lib/systemd/system/systemd-userdbd.socket
CopyFile /etc/systemd/system/syncthing@.service.d/override.conf
CreateLink /etc/systemd/system/sysinit.target.wants/systemd-timesyncd.service /usr/lib/systemd/system/systemd-timesyncd.service
CreateLink /etc/systemd/system/systemd-rfkill.service /dev/null
CreateLink /etc/systemd/system/systemd-rfkill.socket /dev/null
CreateLink /etc/systemd/system/timers.target.wants/fstrim.timer /usr/lib/systemd/system/fstrim.timer
CreateLink /etc/systemd/user/graphical-session-pre.target.wants/xdg-user-dirs.service /usr/lib/systemd/user/xdg-user-dirs.service
CreateLink /etc/systemd/user/pipewire-session-manager.service /usr/lib/systemd/user/wireplumber.service
CreateLink /etc/systemd/user/pipewire.service.wants/wireplumber.service /usr/lib/systemd/user/wireplumber.service
CreateLink /etc/systemd/user/sockets.target.wants/p11-kit-server.socket /usr/lib/systemd/user/p11-kit-server.socket
CreateLink /etc/systemd/user/sockets.target.wants/pipewire-pulse.socket /usr/lib/systemd/user/pipewire-pulse.socket
CreateLink /etc/systemd/user/sockets.target.wants/pipewire.socket /usr/lib/systemd/user/pipewire.socket
CopyFile /etc/systemd/zram-generator.conf
CopyFile /etc/texmf/ls-R
CopyFile /etc/texmf/web2c/fmtutil.cnf
CopyFile /etc/texmf/web2c/updmap.cfg
CopyFile /etc/throttled.conf
CopyFile /etc/timeshift/timeshift.json
CreateDir /etc/userdb
CopyFile /etc/vconsole.conf
CreateDir /lost+found 700
CreateDir /opt/containerd/bin 711
CreateDir /opt/containerd/lib 711
CopyFile /var/.updated
CreateFile /var/db/sudo/lectured/1000 600 '' sybil > /dev/null


# Tue Jan 27 07:12:49 PM EST 2026 - New file properties


SetFileProperty /boot/intel-ucode.img mode 755
SetFileProperty /etc/pacman.d/gnupg/crls.d mode 700
SetFileProperty /etc/pacman.d/gnupg/openpgp-revocs.d mode 700
SetFileProperty /etc/pacman.d/gnupg/private-keys-v1.d mode 700
SetFileProperty /opt/containerd mode 711
